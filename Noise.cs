...
namespace SimplexNoise
{
    public class RecursiveNoise
    {
        ...
        public static float Noise3D(float x, float y, float z, int depth, float frequencyFactor = 2f, float amplitudeFactor = 0.5f)
        {
            float value = 0f;
            float amplitude = 1f;
            for (int i = 0; i < depth; i++)
            {
                value += Noise.Generate(x, y, z) * amplitude;
                x *= frequencyFactor;
                y *= frequencyFactor;
                z *= frequencyFactor;
                amplitude *= amplitudeFactor;
